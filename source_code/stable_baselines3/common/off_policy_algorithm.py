...
from stable_baselines3.augmented_policies.policies import AugmentedSACPolicy, AugmentedTD3Policy
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.augmented_policies = (AugmentedTD3Policy, AugmentedSACPolicy)
    ...
    def _setup_model(self, load=False) -> None:
        ...
        if issubclass(self.policy_class, self.augmented_policies):
            # if augmented compute the local controller via environment knowledge
            if load:
                n = self.env.observation_space.shape[0]
                m = self.env.action_space.shape[0]
                local = LocalController(self.env, self.gamma,
                                        loadparams={'doa': th.tensor(0., dtype=th.float64), 'p': th.zeros((n, n)),
                                                    'h': th.zeros((n + m, n + m)), 'xs': th.zeros(n),
                                                    'k': th.zeros((m, n)), 'ubar': th.zeros(m)})
            else:
                local = LocalController(self.env, self.gamma)
            ...
            self.policy.local.to(self.device)
        else:
            self.policy = self.policy_class(
                self.observation_space,
                self.action_space,
    ...
    def _sample_action(
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            if issubclass(self.policy_class, self.augmented_policies):
                # we extract only the learing action
                pi_scaled = self.policy.scale_action(unscaled_action, self._last_obs)
            else:
            ...
            if action_noise is not None:
                if issubclass(self.policy_class, self.augmented_policies):
                    # add noise only to pi for focused exploration
                    pi_scaled = np.clip(pi_scaled + action_noise(), -1, 1)
                else:
                    scaled_action = np.clip(scaled_action + action_noise(), -1, 1)

            # We store the scaled action in the buffer
            if issubclass(self.policy_class, self.augmented_policies):
                # state is needed to compute the full action
                action = self.policy.unscale_action(pi_scaled, self._last_obs)
                scaled_action = action  # save the full action
            else:
