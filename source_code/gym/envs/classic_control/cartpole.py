...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-self.force_mag, high=self.force_mag,
                                       shape=(1,), dtype=np.float32)#spaces.Discrete(2)
    ...
    def step(self, action):
        ...
        self.state = x, angle_normalize(theta), x_dot, theta_dot
    ...
    def reset(self, s0=None):
        if s0 is not None:
            self.state = np.array([s0[0], angle_normalize(s0[1]), s0[2],s0[3]])
        else:
